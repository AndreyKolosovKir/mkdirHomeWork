...
public class Main {
    public static void main(String[] args) {
        ...
        File directory = new File("D://GamesNew");
        ...
        if (directory.mkdir()) {
            ...
            System.out.println("Папка GameNew создана, на диске D, логи записаны");
            if (directory.exists()) {
                ...
                System.out.println("Папка GameNew не существует");
            }
        } else {
            System.out.println("Папка GameNew не создана");
        }

        File fileTemp = new File("D://GamesNew/tems/", "temp.txt");
        if (fileTemp.canWrite()) {
            System.out.println("могу записать файл temp");
        } else {
            System.out.println("файл temp не может быть записан");
        }

        try (FileOutputStream fos = new FileOutputStream("D://GamesNew/tems/temp.txt")) {
    ...
    public static void mkdirInGameNew(List<String> paths, StringBuilder stringBuilder) {
        for (String pathGameNew : paths) {
            File newDirectory = new File("D://GamesNew/" + pathGameNew);
            if (newDirectory.mkdir()) {
                writeLogDir(stringBuilder, newDirectory);
                System.out.println("Папка " + pathGameNew + " создана, логи записаны");
            } else {
                System.out.println("Папка " + pathGameNew + " не создана");
            }
        }
    ...
    public static void mkdirInRes(List<String> pathsInRes, StringBuilder stringBuilder) {
        for (String pathNew : pathsInRes) {
            File newDirectory = new File("D://GamesNew/res/" + pathNew);
            if (newDirectory.mkdir()) {
                writeLogDir(stringBuilder, newDirectory);
    ...
    public static void mkdirInSrc(List<String> pathsInSrc, StringBuilder stringBuilder) {
        for (String pathNew : pathsInSrc) {
            File newDirectory = new File("D://GamesNew/src/" + pathNew);
            if (newDirectory.mkdir()) {
                writeLogDir(stringBuilder, newDirectory);
                System.out.println("Папка " + pathNew + " создана, логи записаны");
            } else {
    ...
    public static void newFiles(List<String> files, StringBuilder stringBuilder) {
        for (String file : files) {
            File newFile = new File("D://GamesNew/src/main/", file);
            File newFileTemp = new File("D://GamesNew/tems/", file);
            try {
